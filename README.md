...
|---|---|
| **a GTK4 application** | GTK4's GL renderer needs an **OpenGL 3.2** host context. A host whose Mesa cannot create one, such as Ubuntu 14.04's Mesa 10.1 with a modern glvnd dispatcher, makes GTK4 fall back to Cairo rendering and GL-using widgets report "GL disabled". GTK4 itself still runs; the preload cannot manufacture a GL context the host's Mesa will not create. Measured on `ubuntu:14.04` (falls back) and on `ubuntu:16.04` and softpipe (GL 3.3), where the GL renderer works, recorded in [`docs/limits.md`](docs/limits.md) |
