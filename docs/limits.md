...
| **A GTK4 GL renderer needs an OpenGL 3.2 host context; a host whose GL stack cannot provide one falls back to Cairo and GL widgets report "GL disabled"** | GTK4's GL renderer asks for a minimum of OpenGL 3.2 (or GLES 2.0), and it works wherever that exists: measured on `ubuntu:16.04` (Mesa 18.0.5), on softpipe (GL 3.3) and on Mesa 26.1.4. The one failure seen is `ubuntu:14.04` (Mesa 10.1), where the renderer could not realize a GL context and GTK4 fell back to `GskCairoRenderer`, with the OpenGLArea demo reporting GL disabled. The ceiling is the host's context, not a missing extension. A shim forwards an entry point; it cannot manufacture a GL context the host's Mesa will not create |
